...
class MultiGetForm(forms.Form):
    ...
    def submit(self, request):
        ...
        if self.cleaned_data['is_relationshiptype'] is not None:
            qbuild.filter('is_relationshiptype', self.cleaned_data['is_relationshiptype'], check=False)

        if self.cleaned_data['is_obsolete'] is not None:
            qbuild.filter('is_obsolete', self.cleaned_data['is_obsolete'], check=False)
